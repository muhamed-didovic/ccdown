...
export default class Crawler {
    ...
    static async getCourses(searchFromLocalFile) {
        ...
        const options = {
            //json     : true,
            'method' : 'POST',
            'url'    : 'https://edge.meilisearch.com/indexes/courses/search',//'https://ms.codecourse.com/indexes/courses/search',
            'headers': {
    ...
    async extractData(seriesResponse, opts, course, i, prefix, courses = []) {
        ...
        const [, res] = await Promise.all([
            ...
            (async () => {
                ...
                ms.update(course.slug, { text: `Extracting started ${ i }/${ courses.length } course: ${ course.slug } has ${ seriesResponse?.props?.parts?.data?.length } episodes` });
                const r = await Promise
                    .map(seriesResponse.props.parts.data, async (s) => {
                        // logger.log('2222222:', `${this.url}/watch/${s.course_slug}?part=${s.slug}`);
                        ms.update(course.slug, { text: `Extracting continues ${ i }/${ courses.length } course: ${ course.slug } has ${ i++ }/${ seriesResponse?.props?.parts?.data?.length } episodes, scraping url: ${ this.url }/watch/${ s.course_slug }?part=${ s.slug }` });
                        ...
                        if (body === 'error code: 1015') {
                            ...
                            logger.log('currentPart', body?.props?.currentPart?.data);//?.video?.data
                        }
