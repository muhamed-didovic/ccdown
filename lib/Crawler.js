        ...
        const [, res] = await Promise.all([
            ...
            (async () => {
                ...
                const r = await Promise
                    .map(seriesResponse.props.parts, async (s) => {
                        logger.log('2222222:', `${this.url}/watch/${s.course_slug}?part=${s.slug}`);
                        ms.update(course.slug, { text: `Extracting continues ${ i }/${ courses.length } course: ${ course.slug } has ${ i++ }/${ seriesResponse?.props?.parts?.length } episodes, scraping url: ${ this.url }/watch/${ s.course_slug }?part=${ s.slug }` });
                        ...
                        if (body === 'error code: 1015') {
                            ...
                            logger.log('currentPart', body?.props?.currentPart);//?.video?.data
                        }
