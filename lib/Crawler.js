...
module.exports = class Crawler {
            ...
            .then(async (courses) => {
                ...
                return await Promise
                    .map(courses, async (course, index) => {
                        ...
                        return await this.slowForever(async () => await this.extractData(seriesResponse, course, i++, courses, opts))
                    }, {
                        concurrency: 1
                    })
                    .then(c => {
    ...
    async extractData(seriesResponse, course, i, courses, opts) {
            ...
            (async () => {
                ...
                const r = await Promise
                    .map(seriesResponse.props.parts.data, async (s) => {
                        ms.update(course.slug, { text: `Extracting ${i}/${courses.length} course: ${course.slug} has ${seriesResponse?.props?.parts?.data?.length} episodes, scraping url: ${this.url}/watch/${s.course_slug}?part=${s.slug}` });
    ...
    async d(filename, prefix, courses, opts) {
        const { logger, concurrency, file, filePath, ms } = opts
