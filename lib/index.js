...
exports.all = async (opts = {}) => {
    ...
    let courses = file ? require(filePath) : await crawler.getAllCourses({ ...opts, ms }) //await logger.promise(crawler.getAllCourses(opts), 'Finding all courses from /subjects endpoint..')
...
exports.one = async (url, opts = {}) => {
    ...
    // get single course
    const singleCourse = await crawler.getSingleCourse({ url, ms }) //await logger.promise(crawler.getSingleCourse(url), 'Finding course videos..')
    ...
    console.log(`Courses and videos are saved in file: ${path.join(process.cwd(), 'json/single-cours-${data}.json')}`);
