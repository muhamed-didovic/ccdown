...
exports.all = async (opts = {}) => {
    ...
    const courses = await logger.promise(crawler.getAllCourses(), "Finding all courses from /library/all endpoint..")
    ...
    await Promise.map(courses, async (course) => {
        ...
        let url= await crawler._vimeoRequest(course.url)
    ...
    logger.succeed(`Downloaded all videos from /library/all api! (total: ${cnt})`)
}
