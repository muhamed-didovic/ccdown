...
`, {
    ...
    flags: {
        ...
        videos     : { type: 'string', shortFlag: 'v', default: 'yes' },
    }
})
    ...
    if (input.length === 0 && searchOrDownload === false) {
        ...
        async function selectMultipleCourses() {
            ...
            const answers = await askOrExit({
                ...
                message: 'Search for a course',
                choices:[...(await Crawler.getCourses(foundSearchCoursesFile))].map(item => ({ ...item, title: `[${item.id}] ${item.title}` })),
                suggest: (input, choices) => {
                    console.log('111');
                    if (!input) return choices;
        ...
        if (input.length === 0) {
            const a = await selectMultipleCourses()
            console.log('selectMultipleCourses:', a);
            input.push(a)
        }
        // input.push(await askOrExit({
