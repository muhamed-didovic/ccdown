...
const newDownload = (url, dest, localSizeInBytes, remoteSizeInBytes, downFolder, index = 0, ms, name) => {
    return new Promise(async (resolve, reject) => {
        ...
        await ytdown(url, dest, ms, index, localSizeInBytes, remoteSizeInBytes, logger, downFolder, name, reject, resolve)
    });
};
...
export default async (file, dest, {downFolder, index, ms} = {}) => {
    ...
    const name = dest + generateRandomString(7)
    ...
    if (isDownloaded) {// && overwrite === 'no'
        ms.succeed(name, {text: `${index}. Resource already downloaded: ${dest.split('/').pop()} - ${localSizeInBytes}/${formatBytes(remoteFileSize)}`});
        //ms.remove(dest);
        logger.debug(`[downOverYoutubeDL] ${index}. Resource already downloaded: ${dest.split('/').pop()} - ${localSizeInBytes}/${formatBytes(remoteFileSize)}`.blue);
        return;
    } else {
