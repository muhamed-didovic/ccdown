...
const ytdown = async (url, dest, ms, index, localSizeInBytes, remoteSizeInBytes, logger, downFolder, name, reject, resolve) => {
    return await retry(async () => {//return
        ytDlpWrap
            ...
            .on("error", (error) => {
                logger.warn('--------error-----', error.statusCode)
                // logger.error('arguments:', arguments)
                //
                if (!error.message.includes('HTTP Error 404')) {
                    logger.error('404 URL:', url, 'dest:', dest, 'error--', error)
                }
